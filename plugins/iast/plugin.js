...
var algorithmConfig = {
    // 防止前端报错
    meta: {}, iast: {
        fuzz_server: 'http://127.0.0.1:25931/openrasp-result', request_timeout: 5000, byhost_regex: '.*'
    }
}
...
function send_rasp_result(context) {
    ...
    if (hook_info.length == 0 && context.header["scan-request-id"] === undefined && context.header["content-type"] != undefined && context.header["content-type"].indexOf("application/xml") < 0) {
        return
    }
    ...
    if (!server_host) {
        let msg = "Agent with rasp id:" + context.raspId + " get host from http header failed! "
        plugin.log(msg)
        return
    }
    if (byhost_regex && byhost_regex.test(server_host)) {
        ...
        for (var i in context.nic) {
            if (context.nic[i].ip && ip_regex.test(context.nic[i].ip) && context.nic[i].ip != "127.0.0.1") {
                web_server.host = context.nic[i].ip
                break
            }
        }
        if (!web_server.host) {
            let msg = "Agent with rasp id:" + context.raspId + " get ip failed! "
            plugin.log(msg)
            return
        } else {
            web_server.port = parseInt(server_host[1]) || default_port
    ...
    // 将hook点信息发送给扫描服务器
    var data = {
        "web_server": web_server, "context": new_context, "hook_info": hook_info, "plugin_version": plugin_version
    }

    var request_config = {
        "method": "post", "data": data, "timeout": algorithmConfig['iast']['timeout'], "maxRedirects": 0, "headers": {
